...
const sendWithConcurrency = async (clients, limit) => {
  ...
  for (const client of clients) {
    if (!isClientEligible(client)) {
      // ✅ Also log skipped clients with a body
      try {
        await SmsLog.create({
          clientId: client._id,
          clientName: client.name,
          clientPhone: client.phone,
          sequenceIndex: client.campaign?.nextSequenceIndex ?? 0,
          sequenceLabel: "skipped",
          body: `Skipped: Client not eligible for campaign`,
          status: "skipped",
          sentAt: new Date(),
        });
      } catch (logErr) {
        logger.error(`[LOG-ERROR] Failed to log skip for ${client.name}: ${logErr.message}`);
      }
  ...
  if (client) {
    ...
    try {
      await SmsLog.create({
        ...
        clientPhone: client.phone,
        sequenceIndex: -1,
        sequenceLabel: "opt-out",
        body: `Client opted out of campaign`,
        status: "skipped",
        sentAt: new Date(),
      });
    } catch (logErr) {
      logger.error(`[LOG-ERROR] Failed to log opt-out for ${client.name}: ${logErr.message}`);
    }
  } else {
